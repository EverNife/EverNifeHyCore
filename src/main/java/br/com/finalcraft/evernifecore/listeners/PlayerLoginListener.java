...
public class PlayerLoginListener implements ECListener {
    ...
    public void onRegister() {
        EverNifeCore.instance.getEventRegistry().registerGlobal(PlayerSetupConnectEvent.class, (event) -> {
            handlePlayerAsyncPreUUIDToNameCalculation(event.getUuid(), event.getUsername());
        });
    ...
    public static void handlePlayerAsyncPreUUIDToNameCalculation(UUID currentUUID, String currentName){
        ...
        if (!currentUUID.equals(offlineCalculatedUUID) // Scenario [C1], we are online
            ...
            playerData.getConfig().save(new File(FCJavaPluginUtil.getDataFolder(EverNifeCore.instance), "PlayerData-Dormant/" + existingUUID + ".yml"));//Move to dormant folder
