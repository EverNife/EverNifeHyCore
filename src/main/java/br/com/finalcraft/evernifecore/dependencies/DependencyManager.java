...
public class DependencyManager extends LibraryManager {
    ...
    public DependencyManager(JavaPlugin plugin) {
        super(new JDKLogAdapter(Logger.getLogger(Objects.requireNonNull(plugin, "plugin").getName())), FCJavaPluginUtil.getDataFolder(plugin).toPath());
        classLoader = new URLClassLoaderHelper((URLClassLoader) plugin.getClass().getClassLoader(), this);
    }

    public DependencyManager(JavaPlugin plugin, Path dataDirectory, String folderName) {
        super(new JDKLogAdapter(Logger.getLogger(Objects.requireNonNull(plugin, "plugin").getName())), dataDirectory, folderName);
        classLoader = new URLClassLoaderHelper((URLClassLoader) plugin.getClass().getClassLoader(), this);
    }
